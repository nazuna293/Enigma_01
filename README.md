...
+ 1.0u配列でchoc v1を使用する場合は、1箇所ピンカットする必要があります。
  > 1.5u配列、2.0u配列ではピンカットは不要、そのままご使用いただけます。  
  > PCBソケットを取り外すことでピンカットは不要なため事前にお申し付けください。
+ いずれの配列の場合でも、最下段左から5番目のスイッチが横向きに付く関係で、choc v1 **キーキャップの印字も横向き**になります。
+ 縦向き1.5uのキーキャップが存在しないため、choc v1で1.5u配列は使用できません。
